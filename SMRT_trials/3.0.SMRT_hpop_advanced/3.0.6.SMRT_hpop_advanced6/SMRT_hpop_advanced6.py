...
Here, hyperparameter optimization is run with early stopping mechanism implemented. Search space:
    * ffn_hidden_dim = [400, 700] (log = True)
    * mp_hidden_dim = 450
    * ffn_layers = 4
    * dropout_rate = 0.1
    * final_lr = [1e-8, 1e-2, log=True]
...
def objective(trial):
    ...
    ffn_hidden_dim = trial.suggest_int("ffn_hidden_dim", 400, 700, log=True)
    final_lr = trial.suggest_float("final_lr", 1e-8, 1e-2, log=True)
    ...
    model = models.MPNN(mp, agg, ffn, batch_norm, metric_list, final_lr=final_lr)
...
path2file = "/home/user/YixiTFG/TFG_Yixi/SMRT_trials/3.0.SMRT_hpop_advanced/3.0.6.SMRT_hpop_advanced6/"
os.makedirs (path2file, exist_ok = True)
filename = "Results_hpop_advanced6.tsv"
sorted_results.to_csv (path2file + filename, sep='\t')
